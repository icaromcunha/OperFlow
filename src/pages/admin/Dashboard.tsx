...
export default function AdminDashboard({ user }: { user: any }) {
  ...
  const isConsultant = user?.perfil === 'consultor';
  ...
  const alerts = clients.slice(0, 3).map((c, i) => ({
    id: i + 1,
    clientId: c.id,
    client: c.nome,
    issue: i === 0 ? "Queda de vendas (-30%)" : i === 1 ? "Erro de integração Shopee" : "Atraso logístico (Coleta)",
    type: i === 0 ? "sales" : i === 1 ? "integration" : "logistics",
    severity: i === 0 ? "crítico" : i === 1 ? "alto" : "médio"
  }));
        ...
        <div className="lg:col-span-2 space-y-8">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-black text-text-primary flex items-center gap-2 italic uppercase tracking-tight">
                <Zap className="text-brand-orange size-5" />
                Fila de Protocolos Prioritários
              </h2>
              <button 
                onClick={() => navigate("/admin/global-queue")}
                className="text-xs font-black uppercase tracking-widest text-brand-orange hover:text-brand-orange/80 transition-colors"
              >
                Ver Todos
              </button>
            </div>
            ...
            <div className="bg-bg-card rounded-2xl border border-border-main shadow-sm overflow-hidden">
              <div className="overflow-x-auto">
                <div className="min-w-[800px]">
                  {/* Header */}
                  <div className="grid grid-cols-[100px_1fr_1.5fr_100px_100px_120px] px-6 py-4 border-b border-border-main bg-surface-subtle/50">
                    <span className="text-[10px] font-black uppercase tracking-widest text-text-secondary opacity-60">ID</span>
                    <span className="text-[10px] font-black uppercase tracking-widest text-text-secondary opacity-60">Cliente</span>
                    <span className="text-[10px] font-black uppercase tracking-widest text-text-secondary opacity-60">Assunto</span>
                    <span className="text-[10px] font-black uppercase tracking-widest text-text-secondary opacity-60">Prioridade</span>
                    <span className="text-[10px] font-black uppercase tracking-widest text-text-secondary opacity-60">SLA</span>
                    <span className="text-[10px] font-black uppercase tracking-widest text-text-secondary opacity-60 text-right">Status</span>
                  </div>
                  ...
                  <div className="divide-y divide-border-main">
                    {protocols.slice(0, 5).map((p) => (
                      <div 
                        ...
                        className="grid grid-cols-[100px_1fr_1.5fr_100px_100px_120px] px-6 py-4 items-center hover:bg-surface-hover transition-colors cursor-pointer group"
                      >
                        <span className="text-xs font-mono text-text-secondary group-hover:text-brand-orange transition-colors">#PRT-{p.id}</span>
                        <span className="text-sm font-bold text-text-primary truncate pr-4">{p.cliente_nome}</span>
                        <span className="text-sm text-text-secondary font-medium truncate pr-4">{p.titulo || "Análise de Conta"}</span>
                        <div>
          ...
          <div className={`grid grid-cols-1 ${isConsultant ? '' : 'md:grid-cols-2'} gap-6`}>
            {!isConsultant && (
            ...
            <div className="bg-bg-card p-6 rounded-3xl border border-border-main shadow-sm">
              ...
              <div className="space-y-4">
                {[
                  ...
                  { label: "Outros", value: 10, color: "bg-surface-subtle" }
                ].map((cat, i) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <div className="relative">
                          <div className="size-10 rounded-full bg-surface-subtle flex items-center justify-center text-text-secondary font-bold group-hover:bg-brand-orange/10 group-hover:text-brand-orange transition-colors">
